...
- Полиморфизм во время выполнения — через virtual/override
...
Модификатор, позволяющий передать переменное число аргументов одного типа. Внутри метода они представлены массивом.
...
**Boxing** — упаковка value type в object (в куче). Создаётся обёртка, данные копируются.

**Unboxing** — распаковка object обратно в value type.
...
Boxing/unboxing создают нагрузку; лучше использовать generics.

---

### 25. Что такое string immutability?

Строки в C# неизменяемы. Любая операция, «изменяющая» строку, создаёт новый объект. Поэтому для интенсивной конкатенации предпочтителен `StringBuilder`.

---

### 26. StringBuilder — когда использовать?

Когда нужно много раз модифицировать строку (циклы, конкатенация). `StringBuilder` работает с внутренним буфером и избегает создания множества объектов строк.
...
Тип, представляющий ссылку на метод. Позволяет передавать методы как аргументы и вызывать их. Основа для событий.
...
**==** — оператор сравнения. Для reference types по умолчанию сравнивает ссылки. Для value types — значения. Может быть перегружен.

**Equals()** — виртуальный метод. Определяет семантическое равенство. Рекомендуется переопределять вместе с `GetHashCode()`.

---

### 31. Зачем переопределять GetHashCode() при Equals()?

Контракт: если два объекта равны по `Equals()`, они должны возвращать одинаковый `GetHashCode()`. Иначе ломается работа `Dictionary`, `HashSet` и других коллекций, использующих хеш.
...
Проверка значения и извлечение данных через шаблоны: `is`, `switch` expression, `when`, `and`, `or`, типы и т.д.
...
Директива `using` гарантирует вызов `Dispose()` после выхода из блока, даже при исключении. Подходит для файлов, соединений, и т.п.
...
### 96. Background Services — как создать?
...
- **Всего вопросов / тем:** 290+ (включая чеклист и блоки 19–34)
- **Блоки:** 34
- **Темы:** C#, .NET, ASP.NET Core, SQL Server, React, EF Core, тестирование, Git, DevOps, soft skills, память, SOLID, Clean Architecture, CQRS, Security, Production, HTTP/REST, Caching, Redis, Design Patterns, Mapping, Pagination, API Design, Resilience, Observability, тестирование (глубже), C# новые фичи, EF (Value Objects, Interceptors), HttpClient, микросервисы (Gateway, Service Discovery), record struct, NRT, Result pattern
