...
**Пример рассказа про свой опыт (подставь свой проект, например EPY S.L.R.):** «На проекте мы используем Azure DevOps Pipelines. При push в main запускается сборка: restore, build, publish в артефакт. Параллельно прогоняются unit-тесты; при падении сборка помечается как failed. Успешный артефакт деплоится в окружение Test автоматически. В Production деплой идёт после ручного approval: в Azure DevOps настроен environment Prod с проверкой «approval», чтобы перед выкаткой в прод кто-то из команды подтвердил. Секреты и строки подключения хранятся в переменных пайплайна (secret variables), не в коде.»
...
#### OAuth 2.0 и OpenID Connect (базово)

- **OAuth 2.0** — протокол авторизации: «дать приложению доступ к ресурсам без передачи пароля». Роли: Resource Owner (пользователь), Client (приложение), Authorization Server (выдаёт токены), Resource Server (API). Grants: Authorization Code (с PKCE для SPA), Client Credentials (сервис-сервис), Refresh Token.
- **OpenID Connect (OIDC)** — слой поверх OAuth 2.0: добавление идентификации (кто пользователь). ID Token (JWT) с claims о пользователе; UserInfo endpoint. Используется для «логин через Google/GitHub/Microsoft».

В ASP.NET Core: `AddAuthentication().AddJwtBearer()` для своего API; для входа через провайдера — `AddOpenIdConnect()` или готовые схемы (Microsoft, Google).
